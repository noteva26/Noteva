...
impl CommentRepository for CommentRepositoryImpl {
    async fn create(&self, input: CreateCommentInput, user_id: Option<i64>, ip: Option<String>, ua: Option<String>) -> Result<Comment> {
        match self.pool.driver() {
            DatabaseDriver::Sqlite => create_sqlite(self.pool.as_sqlite().unwrap(), input, user_id, ip, ua).await,
            DatabaseDriver::Mysql => create_mysql(self.pool.as_mysql().unwrap(), input, user_id, ip, ua).await,
        }
    }
...
async fn create_sqlite(pool: &SqlitePool, input: CreateCommentInput, user_id: Option<i64>, ip: Option<String>, ua: Option<String>) -> Result<Comment> {
    let now = Utc::now();
    let result = sqlx::query(
        r#"INSERT INTO comments (article_id, user_id, parent_id, nickname, email, content, ip_address, user_agent, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"#
    )
...
async fn create_mysql(pool: &MySqlPool, input: CreateCommentInput, user_id: Option<i64>, ip: Option<String>, ua: Option<String>) -> Result<Comment> {
    let now = Utc::now();
    let result = sqlx::query(
        r#"INSERT INTO comments (article_id, user_id, parent_id, nickname, email, content, ip_address, user_agent, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"#
