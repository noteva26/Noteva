...
// (Functions that differ from MySQL or cross-reference other _sqlite fns)
// ============================================================================

pub(super) async fn create_article_sqlite(
    pool: &SqlitePool,
    ...
    let scheduled_at = if status == ArticleStatus::Published {
        None
    } else {
        input.scheduled_at
    };
...
pub(super) async fn get_article_by_id_sqlite(
    pool: &SqlitePool,
    id: i64,
...
pub(super) async fn get_article_by_slug_sqlite(
    pool: &SqlitePool,
    slug: &str,
) -> Result<Option<Article>> {
    let row = sqlx::query(
...
pub(super) async fn list_articles_sqlite(
    pool: &SqlitePool,
    offset: i64,
    limit: i64,
...
pub(super) async fn update_article_sqlite(
    pool: &SqlitePool,
    id: i64,
...
pub(super) async fn list_articles_by_category_sqlite(
    pool: &SqlitePool,
    category_id: i64,
    offset: i64,
    limit: i64,
...
pub(super) async fn list_articles_by_tag_sqlite(
    pool: &SqlitePool,
    tag_id: i64,
    offset: i64,
    limit: i64,
...
pub(super) async fn list_published_articles_sqlite(
    pool: &SqlitePool,
    offset: i64,
    limit: i64,
    sort_by: ArticleSortBy,
...
pub(super) async fn list_published_articles_by_category_ids_sqlite(
    pool: &SqlitePool,
    category_ids: &[i64],
    offset: i64,
    limit: i64,
...
pub(super) async fn list_published_articles_by_tag_sqlite(
    pool: &SqlitePool,
    tag_id: i64,
    offset: i64,
    limit: i64,
...
pub(super) async fn list_articles_by_status_sqlite(
    pool: &SqlitePool,
    ...
    let order_prefix = if status == ArticleStatus::Published {
        "is_pinned DESC, pin_order ASC, "
    } else {
        ""
    };
    let query = format!(
...
pub(super) async fn search_articles_sqlite(
    pool: &SqlitePool,
    keyword: &str,
    offset: i64,
    limit: i64,
    ...
    let rows = if use_fts {
        ...
        sqlx::query(&query)
            ...
            .fetch_all(pool)
            .await
            .context("Failed to search articles (FTS5)")?
    } else {
