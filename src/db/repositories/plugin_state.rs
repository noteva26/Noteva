...
async fn get_sqlite(pool: &SqlitePool, plugin_id: &str) -> Result<Option<PluginState>> {
    ...
    Ok(row.map(|r| {
        ...
        PluginState {
            ...
            settings,
            last_version: r
                .try_get::<Option<String>, _>("last_version")
                .unwrap_or(None),
        }
    }))
}
...
async fn get_all_sqlite(pool: &SqlitePool) -> Result<Vec<PluginState>> {
    let rows = sqlx::query("SELECT plugin_id, enabled, settings, last_version FROM plugin_states")
        .fetch_all(pool)
        .await?;
...
async fn update_last_version_sqlite(
    pool: &SqlitePool,
    plugin_id: &str,
    version: &str,
) -> Result<()> {
...
async fn update_last_version_mysql(pool: &MySqlPool, plugin_id: &str, version: &str) -> Result<()> {
    sqlx::query(
        "UPDATE plugin_states SET last_version = ?, updated_at = NOW() WHERE plugin_id = ?",
    )
    .bind(version)
    .bind(plugin_id)
    .execute(pool)
