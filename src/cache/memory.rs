...
mod tests {
            ...
            runner
                .run(&article_strategy, |article| {
                    ...
                    rt.block_on(async {
                        ...
                        assert_eq!(
                            source_call_count.load(Ordering::SeqCst),
                            1,
                ...
                .run(&kv_strategy, |kv_pairs| {
                    ...
                    rt.block_on(async {
                        ...
                        for (key, _) in &kv_pairs {
                            ...
                            assert_eq!(
                                result, None,
                                "Value should be expired after TTL for key: {}",
                                key
                            );
                        }
                    });
    ...
    async fn test_complex_types() {
        ...
        cache
            .set("article:1", &article, Duration::from_secs(60))
            .await
            .unwrap();
