...
pub async fn restore_backup(
    ...
    let manifest: BackupManifest = {
        let mut f = archive
            ...
            .context("Missing manifest.json in backup")?;
...
pub async fn import_articles(
    ...
    if data.len() >= 4 && &data[0..4] == b"PK\x03\x04" {
        ...
        && std::str::from_utf8(&data[..100.min(data.len())]).map_or(false, |s| {
            s.contains("<?xml") || s.contains("<rss") || s.contains("<wp:")
        })
    {
        import_wordpress_xml(pool, data, author_id).await
    } else {
        anyhow::bail!(
            "Unsupported file format. Expected a ZIP file (Markdown) or XML file (WordPress WXR)."
        )
    }
}

/// Import articles from a ZIP of Markdown files with YAML frontmatter
async fn import_markdown_zip(
    pool: &DynDatabasePool,
    zip_data: &[u8],
    author_id: i64,
) -> Result<ImportResult> {
    ...
    for (name, content) in &entries {
        ...
        match insert_article(pool, &slug, &title, &body, &html, author_id, &status).await {
            ...
            Err(e) => {
                result
                    .errors
                    .push(format!("Failed to import '{}': {}", title, e));
            }
        }
    }
...
async fn import_wordpress_xml(
    ...
    for item in xml_str.split("<item>").skip(1) {
        ...
        let slug = extract_xml_tag(item, "wp:post_name").unwrap_or_else(|| slugify(&title));
        ...
        let status = if status_str == "publish" {
            "published"
        } else {
            "draft"
        };
...
async fn check_slug_exists(pool: &DynDatabasePool, slug: &str) -> bool {
    match pool.driver() {
        DatabaseDriver::Sqlite => {
            let Ok(p) = pool.as_sqlite_or_err() else {
                return false;
            };
        ...
        DatabaseDriver::Mysql => {
            let Ok(p) = pool.as_mysql_or_err() else {
                return false;
            };
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM articles WHERE slug = ?")
                .bind(slug)
