      ...
      <div className="relative flex min-h-screen flex-col">
        ...
        <main className="flex-1">
          <div className="container py-8 max-w-4xl mx-auto">
            <div className="mb-8">
              <Link to="/tags"><Button variant="ghost" size="sm" className="mb-4"><ArrowLeft className="h-4 w-4 mr-2" />{t("common.back")}</Button></Link>
              <div className="flex items-center gap-3 mb-2"><TagsIcon className="h-8 w-8 text-primary" /><h1 className="text-3xl font-bold">#{selectedTag.name}</h1></div>
    ...
    <div className="relative flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <div className="container py-8 max-w-4xl mx-auto">
          ...
          {loading ? (
            <div className="flex flex-wrap gap-3">{[1,2,3,4,5,6,7,8].map(i => <Skeleton key={i} className="h-10 w-24" />)}</div>
          ) : tags.length === 0 ? (
            <Card><CardContent className="py-12 text-center text-muted-foreground"><TagsIcon className="h-12 w-12 mx-auto mb-4 opacity-50" />{t("tag.noTags")}</CardContent></Card>
          ) : (
            <div className="flex flex-wrap gap-3">
              {tags.map((tag) => (
                <Link key={tag.id} to={`/tags?t=${tag.slug}`} className="inline-flex items-center gap-2 px-4 py-2 bg-card border rounded-full hover:border-primary hover:text-primary transition-colors">
