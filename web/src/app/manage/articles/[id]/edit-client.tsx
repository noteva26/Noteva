...
import { ArrowLeft, Save, Eye, X, Trash2, Bold, Italic, Link, Image as ImageIcon, List, ListOrdered, Quote, Code, Heading1, Heading2, Pin } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function getArticleIdFromPath(): number {
  ...
  const match = path.match(/\/manage\/articles\/(\d+)/);
...
export default function EditArticleClient() {
  ...
  // Extract images from content for thumbnail selection
  ...
  const handleSubmit = async (status?: "draft" | "published" | "archived") => {
    if (!form.title.trim()) {
      toast.error(t("article.title") + " " + t("common.error"));
      return;
    }
  ...
  const fetchPreview = async () => {
    if (!form.content.trim()) {
      setPreviewHtml("<p class='text-muted-foreground'>暂无内容</p>");
      return;
    }
    try {
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent>
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder={t("article.category")} />
                </SelectTrigger>
                <SelectContent>
                  {categories.map((cat) => (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{t("article.status")}</CardTitle>
            </CardHeader>
            <CardContent>
              <Select
                value={form.status}
              ...
              {form.is_pinned && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, pin_order: parseInt(e.target.value) || 0 }))}
                    placeholder="0"
                  />
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{t("article.thumbnail")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {form.thumbnail && (
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {contentImages.map((img, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => setForm((f) => ({ ...f, thumbnail: img }))}
                        className={`relative aspect-video rounded-md overflow-hidden border-2 transition-colors ${
                          form.thumbnail === img ? "border-primary" : "border-transparent hover:border-muted-foreground/50"
                        }`}
                      >
                        <Image
                          src={img}
